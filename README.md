...
### Вариант C: локальный веб-интерфейс
```bash
cd dolphin-bot
python3 web_ui.py
```

После запуска откроется локальная панель `http://127.0.0.1:8765`, где можно:
- запустить `bot.py` одной кнопкой;
- видеть весь лог бота в реальном времени;
- отвечать на вопросы бота через браузер;
- вставлять многострочный блок `name/order/email/phone` без терминала;
- видеть раздел с кейсами и статусом: на чём остановились, какой дедлайн, сколько уже прошло и сколько ещё ждать до повторного follow-up.

Если браузер не открылся автоматически, просто открой адрес вручную.

### Вариант D: macOS launcher-приложение
```bash
cd dolphin-bot
bash build_macos_app.sh
```

После сборки появится `dist/Support Copilot.app`.
Его можно запускать двойным кликом как обычное приложение на macOS.

Launcher делает следующее:
- поднимает локальную панель `web_ui.py` в фоне;
- открывает интерфейс в браузере автоматически;
- избавляет от ручного запуска через Terminal.
